...
public class DemoThreadPoolActivity extends BaseActivity implements View.OnClickListener {
    ...
    private void initDefaultPool() {
        ...
        ThreadPoolProxyFactory.getDefaultThreadPool().execute(() -> {
    ...
    private void initCachePool() {
        for (int j = 0; j < 20; j++) {
            ThreadPoolProxyFactory.getCacheThreadPool().execute(() -> {
                Message msg = Message.obtain();
                msg.what = CACHE_POOL;
    ...
    private void initSinglePool() {
        taskIndex.set(1);
        for (int i = 0; i < 5; i++) {
            ThreadPoolProxyFactory.getSingleThreadPool().execute(() -> {
                Message msg = Message.obtain();
    ...
    private void handleMsgFromSingle() {
        singleSB.append("正在处理任务： ").append(taskIndex.getAndIncrement()).append("\n");
