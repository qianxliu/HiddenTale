...
import qian.xin.library.interfaces.ViewPresenter;

import static qian.xin.library.util.StringUtil.getCurrentString;
import static qian.xin.library.util.StringUtil.getTrimedString;
...
public abstract class BaseViewBottomWindow<T, BV extends BaseView<T>> extends BaseBottomWindow
    ...
    public void initData() {//必须调用
        ...
        if (tvBaseTitle != null) {
            ...
            if (!isNotEmpty(title, true)) {
                title = getTitleName();
            }
            tvBaseTitle.setVisibility(isNotEmpty(title, true) ? View.VISIBLE : View.GONE);
            tvBaseTitle.setText(getTrimedString(title));
        }

        if (tvBaseViewBottomWindowReturn != null && isNotEmpty(getReturnName(), true)) {
            tvBaseViewBottomWindowReturn.setText(getCurrentString());
        }
        if (tvBaseViewBottomWindowForward != null && isNotEmpty(getForwardName(), true)) {
            tvBaseViewBottomWindowForward.setText(getCurrentString());
        }
