...
import qian.xin.library.model.GridPickerConfig;
import qian.xin.library.ui.GridPickerView.OnTabClickListener;
...
public class TimePickerWindow extends BaseViewBottomWindow<List<Entry<Integer, String>>, GridPickerView> {
     ...
     * @return
     */
    public static Intent createIntent(AppCompatActivity context, int[] limitTimeDetail) {
     ...
     * @return
     */
    public static Intent createIntent(AppCompatActivity context, int[] limitTimeDetail, int[] defaultTimeDetail) {
     ...
     * @return
     */
    public static Intent createIntent(AppCompatActivity context, int[] minTimeDetail, int[] maxTimeDetail, int[] defaultTimeDetail) {
    ...
    private void setPickerView(final int tabPosition) {
        runThread(TAG + "setPickerView", () -> {
            ...
            runUiThread(() -> containerView.bindView(tabPosition, list));
        });
    }
    ...
    public void initData() {//必须调用
        ...
        if (minTimeDetails.length < MIN_LENGHT || maxTimeDetails.length < MIN_LENGHT) {
            finish();
            return;
        }
        ...
        runThread(TAG + "initData", () -> {
            ...
            runUiThread(() -> containerView.init(configList, list));
        });
    ...
    private synchronized List<Entry<Integer, String>> getList(int tabPosition, ArrayList<Integer> selectedItemList) {
        ...
        switch (level) {
            case TimeUtil.LEVEL_HOUR:
                for (int i = 0; i < 24; i++) {
                    list.add(new Entry<>(getItemType(
...
//					}
                    ...
                    list.add(new Entry<>(getItemType(isEnable), String.valueOf(i)));
                }
                break;
    ...
    protected void setResult() {
        ...
        if (list != null) {
            ...
            for (int i = 0; i < list.size(); i++) {
                detailList.add(Integer.parseInt(StringUtil.getNumber(list.get(i))));
            }
    ...
    private OnTabClickListener onTabClickListener = (tabPosition, tvTab) -> setPickerView(tabPosition);
