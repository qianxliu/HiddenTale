...
import qian.xin.library.manager.CityDB;
...
public class PlacePickerWindow extends BaseViewBottomWindow<List<Entry<Integer, String>>, GridPickerView> {
     ...
     * @return
     */
    public static Intent createIntent(AppCompatActivity context, String packageName, int maxLevel) {
     ...
     * @return
     */
    public static Intent createIntent(AppCompatActivity context, String packageName, int minLevel, int maxLevel) {
    ...
    private void setPickerView(final int tabPosition, final int itemPositon) {
        runThread(TAG + "setPickerView", () -> {
            ...
            runUiThread(() -> containerView.bindView(tabPosition, list, itemPositon));
        });
    }
    ...
    private synchronized List<Entry<Integer, String>> getList(int tabPosition, ArrayList<String> selectedItemList) {
        ...
        if (selectedItemList == null || selectedItemList.size() <= 0 || !PlaceUtil.isContainLevel(level)) {
            return null;
        }
    ...
    private OnTabClickListener onTabClickListener = (tabPosition, tvTab) -> setPickerView(tabPosition, containerView.getSelectedItemPosition(tabPosition));
