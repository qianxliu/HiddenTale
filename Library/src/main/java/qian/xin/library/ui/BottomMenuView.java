...
import qian.xin.library.model.Menu;
...
public class BottomMenuView extends BaseView<List<Menu>> {
    ...
    public BottomMenuView(AppCompatActivity context, int toBottomMenuWindowRequestCode) {
    ...
    public void bindView(final List<Menu> menuList) {
        ...
        if (list.size() > 4) {
            ...
            //弹出底部菜单
            moreMenuNameList = new ArrayList<>();
            moreMenuIntentCodeList = new ArrayList<>();
            Menu moreFsb;
    ...
    private void addItem(final boolean isMoreButton, final int position, final Menu fsb) {
		if (!isMoreButton) {
			if ((position < 0) || (fsb == null) || !StringUtil.isNotEmpty(fsb.getName(), true)
					|| (fsb.getImageRes() <= 0)) {
